...
def keep_alive():
    ...
    while True:
        ...
        for attempt in range(3):  # retry up to 3 times
            try:
                response = requests.get(TARGET_URL1, timeout=5)
                if response.status_code == 200:
                    print(f"✅ Successfully pinged {TARGET_URL1}")
                    success = True
                    break
            except Exception as e:
                print(f"⚠️ Ping attempt {attempt+1} to {TARGET_URL1} failed: {e}")
                time.sleep(2)  # wait before retry

        if not success:
            print(f"❌ Failed to ping {TARGET_URL1} after 3 attempts")
...
def keep_alive2():
    ...
    while True:
        ...
        for attempt in range(3):  # retry up to 3 times
            try:
                response = requests.get(TARGET_URL2, timeout=5)
                if response.status_code == 200:
                    print(f"✅ Successfully pinged {TARGET_URL2}")
                    success = True
                    break
            except Exception as e:
                print(f"⚠️ Ping attempt {attempt+1} to {TARGET_URL2} failed: {e}")
                time.sleep(2)  # wait before retry

        if not success:
            print(f"❌ Failed to ping {TARGET_URL2} after 3 attempts")
...
if __name__ == "__main__":
    ...
    threading.Thread(target=keep_alive2, daemon=True).start()
